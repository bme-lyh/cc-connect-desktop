...
运行 `cc-connect web` 或使用 Windows 快捷方式。启动器会在需要时启动仅监听本机的管理服务，等待就绪后打开带认证信息的本地页面。即使配置中没有项目，程序也会持续运行并进入五步首次向导。

简洁首页采用“一张机器人卡片 = 一个 Agent + 一个工作目录 + 一个平台账号”。向导优先展示 Codex 和 Pi，同时覆盖全部已注册平台；Providers、Skills、Cron 和多平台项目仍保留在“高级设置”中。机器人 Secret 写入操作系统凭据库，管理接口只返回是否已配置。

在 Windows 上，Agent 版本和登录检测会在后台无窗口运行。工作目录既可以手动输入，也可以调用始终显示在当前界面前方的系统文件夹选择器。模型改为读取当前 Agent 本地模型目录的可刷新下拉列表，不再允许手动填写；读取失败时会在选择框下方显示具体原因。

机器人设置中的“显示回答底部运行信息”控制模型、推理强度、Token、上下文占用和工作目录组成的回答尾注。新建简洁机器人默认关闭；编辑旧机器人时保留原有状态。高级项目也可通过“回答页脚”总开关控制。

同一设置页还可分别控制“思考消息”和“工具进度”。关闭工具进度会同时隐藏工具调用和工具结果，只保留 Agent 的最终回答；这些设置按机器人保存。

点击页面右上角的“退出 cc-connect”，可优雅停止全部机器人和本地后台程序。它不同于“退出登录”：后者只清除浏览器保存的管理令牌。之后可以通过桌面快捷方式或 `cc-connect web` 重新启动。

保存机器人前会校验工作目录、Agent 和平台字段，再原子写入配置并在 Windows 上无命令行窗口地受控重启。若托管配置无法加载，程序会恢复 `config.toml.previous`。停用或移除机器人不会删除会话数据及已保存凭据。
